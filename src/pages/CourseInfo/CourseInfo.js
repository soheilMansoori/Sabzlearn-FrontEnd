...
export default function CourseInfo() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8080/v1/courses/related/${courseName}`)
      .then((res) => res.json())
      .then((allData) => {
        // console.log(allData);
  ...
  function getCourseDetails() {
    ...
    fetch(`http://localhost:8080/v1/courses/${courseName}`, {
      headers: {
        Authorization: `Bearer ${localStorageData ? null : localStorageData.userToken}`,
      },
    })
      .then((res) => res.json())
      .then((courseInfo) => {
  ...
  const registerInCourse = (course) => {
    if (course.price === 0) {
      swal({
        ...
        if (result) {
          fetch(`http://localhost:8080/v1/courses/${course._id}/register`, {
            method: "POST",
            headers: {
        ...
        if (result) {
          swal({
            ...
            if (code === null) {
              ...
              fetch(`http://localhost:8080/v1/offs/${code}`, {
                method: "POST",
                headers: {
                  Authorization: `Bearer ${JSON.parse(localStorage.getItem("user")).userToken}`,
                  "Content-Type": "application/json",
                ...
                .then((code) => {
                  fetch(
                    `http://localhost:8080/v1/courses/${course._id}/register`,
                    {
                      method: "POST",
                      headers: {
                        Authorization: `Bearer ${JSON.parse(localStorage.getItem("user")).userToken}`,
                    ...
                    {courseDetails.isUserRegisteredToThisCourse ? (
                      <span className="course-info__register-title">
                        ...
                        >
                          ثبت نام در دوره
                          {' '}
                          {courseDetails.price && courseDetails.price.toLocaleString()}
                        </span>
                    )}
                  </div>
                </div>
                ...
                {relatedCourses.length !== 0 && (
                  <div className="course-info">
                    ...
                    <ul className="course-info__courses-list">
                      {relatedCourses.length && relatedCourses.map((course) => (
                        <li className="course-info__courses-list-item">
                          ...
                          >
                            <img
                              src={`http://localhost:8080/courses/covers/${course.cover}`}
                              // src={course.cover}
                              alt="Course Cover"
